...
%Some of the challenges in applying a TLED simulation model to surgical intervention includes:
%TLED approach addresses some of the challenges in accurately modelling the response of organic materials that may be experienced in simulation, namely:
%\begin{itemize}
%\item Computational grids must be obtained from standard diagnostic medical images
%Meshless does not need surfaces
%\item Human tissues undergo very large strains in the vicinity of contact with a surgical tool %MTLED methods gives reliable results for compressive strains exceeding 70\%
%\item surgical manipulation involves not only large deformations of soft tissue but also cutting and damage. 
%\end{itemize}
%Meshless Total Lagrangian explicit dynamics (MTLED) addresses these issues \cite{Joldes2019} with boundary conditions and as such is considered in the formulation of pre-processors.
